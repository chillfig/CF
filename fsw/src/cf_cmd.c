...
{
    CFE_EVS_SendEvent(CF_EID_INF_CMD_NOOP, CFE_EVS_EventType_INFORMATION, "CF: No-Op received, Version %d.%d.%d.%d %s",
...
{
    bool close_chan = context->action;
    CFE_Status_t status = CF_ERROR;
    CF_Channel_t *chan_ptr = NULL;
    CF_ChannelConfig_t *chan_cfg_ptr = NULL;
    ...
    /* Increment the count of affected channels */
    ++context->count;

    if (chan_cfg_ptr->connection_type != CF_UDP_CHANNEL)
    {
        context->type_fail = 1;
        goto CF_DoOpenClose_Chan_Exit_Tag;
    }

    if (CF_AppData.hk.channel_hk[chan_num].channel_closed == context->action)
    {
        context->same = 1;
        goto CF_DoOpenClose_Chan_Exit_Tag;
    }
    ...
    if (close_chan == true)
    {
        (void)CF_UDP_CleanupConnection(chan_ptr);
    }
    else
    {
        status = CF_UDP_InitConnection(&chan_cfg_ptr->udp_config, chan_ptr);
        if (status != CFE_SUCCESS)
        {
            /* Error, socket init failed */
            CFE_EVS_SendEvent(CF_EID_ERR_INIT_UDP, CFE_EVS_EventType_ERROR,
                              "CF: open cmd: failed to initialize UDP connection for channel %d, returned 0x%08lx", 
                              chan_num, (unsigned long)status);
            context->init_fail = 1;
            goto CF_DoOpenClose_Chan_Exit_Tag;
        }
    }

    /* Update the channel_closed flag */
    CF_AppData.hk.channel_hk[chan_num].channel_closed = close_chan;

CF_DoOpenClose_Chan_Exit_Tag:
    return;
}
...
{
    /* ok to not bounds check action, because the caller is using it in two places with constant values 0 or 1 */
    static const char *          msgstr[]   = {"open", "close"};
    static const char *          eventstr[] = {"open", "closed"};
    CF_ChanAction_OpenCloseArg_t args       = {0, 0, 0, 0, action};

    CF_DoChanAction(cmd, msgstr[action], (CF_ChanActionFn_t)CF_DoOpenClose_Chan, &args);

    /*
     * Note that this command may affect multiple channels, depending on the value of the "chan" argument.
     * When acting on multiple channels, the "same" and "type_fail" outputs do not apply.  In reality all it means is
     * that one of the affected channels was already set that way, or was not a UDP channel. The "init_fail" output
     * applies even if we act on multiple channels.
     */
    if (args.count == 0)
    {
        /* No channels affected, channel parameter error, event already generated by CF_DoChanAction */
        ++CF_AppData.hk.counters.err;
        goto CF_DoOpenClose_Exit_Tag;
    }
    else if (args.init_fail == 1)
    {
        /* At least one channel failed to init UDP, event already generated in CF_DoOpenClose_Chan */
        ++CF_AppData.hk.counters.err;
        goto CF_DoOpenClose_Exit_Tag;
    }
    else if (args.count == 1 && args.type_fail == 1)
    {
        /* A single transaction was matched, and it was not a UDP channel */
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_CONN_TYPE, CFE_EVS_EventType_ERROR,
                          "CF: %s cmd: channel is not a UDP channel", msgstr[action]);
        ++CF_AppData.hk.counters.err;
        goto CF_DoOpenClose_Exit_Tag;
    }
    
    if (args.count == 1 && args.same == 1)
    {
        /* A single transaction was matched, and it was already in the given action state */
        CFE_EVS_SendEvent(CF_EID_INF_CMD_OPENCLOSE, CFE_EVS_EventType_INFORMATION,
                          "CF: %s cmd: UDP channel already %s", msgstr[action], eventstr[action]);
    }
    else
    {
        /* Otherwise, generic success event */
        CFE_EVS_SendEvent(CF_EID_INF_CMD_OPENCLOSE, CFE_EVS_EventType_INFORMATION,
                      "CF: %s cmd: %s UDP channel successful", msgstr[action], msgstr[action]);
    }

    ++CF_AppData.hk.counters.cmd;

CF_DoOpenClose_Exit_Tag:
    return;
}
...
{
    CF_DoOpenClose((CF_UnionArgsCmd_t *)msg, 0);
}
...
{
    CF_DoOpenClose((CF_UnionArgsCmd_t *)msg, 1);
}
...
void CF_ProcessGroundCommand(CFE_SB_Buffer_t *msg)
{
    static void (*const fns[CF_NUM_COMMANDS])(CFE_SB_Buffer_t *) = {
        ...
        CF_CmdOpenUDPChannel,  /* CF_OPEN_UDP_CHANNEL_CC */
        CF_CmdCloseUDPChannel, /* CF_CLOSE_UDP_CHANNEL_CC */
    };
    ...
    static const uint16 expected_lengths[CF_NUM_COMMANDS] = {
        ...
        sizeof(CF_UnionArgsCmd_t),  /* CF_OPEN_UDP_CHANNEL_CC */
        sizeof(CF_UnionArgsCmd_t),  /* CF_CLOSE_UDP_CHANNEL_CC */
    };
