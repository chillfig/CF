...
static void UT_CFDP_SetupBasicRxState(CF_Logical_PduBuffer_t *pdu_buffer)
{
    ...
    /* Ensure the state is reset for all functions we set below */
    UT_ResetState(UT_KEY(CFE_MSG_GetSize));
    UT_ResetState(UT_KEY(CFE_MSG_GetType));
    UT_ResetState(UT_KEY(CFE_SB_ReceiveBuffer));
...
void Test_CF_CFDP_SB_ReceiveMessage(void)
{
    /* Test case for:
    ...
    UtAssert_VOIDCALL(CF_CFDP_SB_ReceiveMessage(c));
...
void Test_CF_CFDP_SB_Send(void)
{
    /* Test case for:
    ...
    CF_AppData.engine.out.msg = &SBBuf;
...
void Test_CF_CFDP_SB_MsgOutGet(void)
{
    /* Test case for:
    ...
    UT_CFDP_SetupBasicTestState(UT_CF_Setup_NONE, NULL, NULL, NULL, &t, NULL);
    UtAssert_NULL(CF_CFDP_SB_MsgOutGet(t, true));
    UtAssert_STUB_COUNT(CFE_EVS_SendEvent, 0);
}
...
{
    UtTest_Add(Test_CF_CFDP_SB_ReceiveMessage, cf_cfdp_tests_Setup, cf_cfdp_tests_Teardown, "CF_CFDP_SB_ReceiveMessage");
    UtTest_Add(Test_CF_CFDP_SB_MsgOutGet, cf_cfdp_tests_Setup, cf_cfdp_tests_Teardown, "CF_CFDP_SB_MsgOutGet");
    UtTest_Add(Test_CF_CFDP_SB_Send, cf_cfdp_tests_Setup, cf_cfdp_tests_Teardown, "CF_CFDP_SB_Send");
}
